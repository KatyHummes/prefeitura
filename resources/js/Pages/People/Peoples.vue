...
<template>
    ...
    <Modal :show="showDeleteUserConfirmModal" @close="closeDeleteUserConfirmModal">
        <form @submit.prevent="deleteUser()" class="bg-[var(--surface-50)]">
            <h2 class="flex items-center justify-center p-4 m-4 font-bold text-[var(--text-color)]">Tem certeza que deseja excluir
                esta Pessoa?</h2>
            <div class="flex justify-around">
                <button type="button" class="bg-[var(--green-500)] text-[var(--primary-color-text)] rounded-md m-4 px-2 py-1"
                    @click="closeDeleteUserConfirmModal">Cancelar</button>
                <button type="submit" class="bg-[var(--red-500)] text-[var(--primary-color-text)] rounded-md m-4 px-2 py-1">Excluir </button>
            </div>
        </form>
    </Modal>
